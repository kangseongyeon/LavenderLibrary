...
@WebServlet("/bookVideo/list.do")
public class ListBookVideoController extends HttpServlet{
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		IBookVideoService bookVideoService = BookVideoServiceImpl.getInstance();
		
		List<BookVideoVO> bookVideoList = bookVideoService.getTotalBookVideo();
		...
		//전체 글 갯수 , 총 데이터 개수
		int totalCount = bookVideoService.countList();
...
//				
//				//범위 별 게시물 가져오기
		List<BookVideoVO> list = bookVideoService.boardList(map);
		...
		req.setAttribute("active", "bookVideoList");
		req.getRequestDispatcher("/views/bookVideo/bookVideoList.jsp").forward(req, resp);
	}
