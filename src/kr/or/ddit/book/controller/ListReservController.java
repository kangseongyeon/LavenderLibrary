...
@WebServlet("/reserv/list.do")
public class ListReservController extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		IBookService bookService=BookService.getInstance();
		
		List<ReservVO> revList=bookService.getAllReserv();
...
//		currentPage = req.getParameter("page") == null ? 1 : Integer.parseInt(req.getParameter("page"));
		...
		//전체 글 갯수 , 총 데이터 개수
		int totalCount = bookService.reservCountList();
		//화면에 표시될 페이지의 수
		int perPage = 5;
...
//								
//				//범위 별 게시물 가져오기
		List<ReservVO> list = bookService.reservBoardList(map);
		...
		req.getRequestDispatcher("/views/reserv/list.jsp").forward(req, resp);
	}
