...
// 						for (RentalVO rv : rentalList) {
					...
					<tr>
						...
						<td><%=ChronoUnit.DAYS.between(rv.getReturnDate(),LocalDate.now())%></td>
			...
			%>
				 <li <% if(i==currentPage) {%> class="active" <% } else {}%>><a href="<%=request.getContextPath() %>/rental/overRental.do?page=<%=i%>"><%=i %></a></li>
			<%		
			}
			%>
	  			</ul>
